...
const COMPLETED_SESSION_CACHE_MS = 30_000;
...
interface CaptureResult {
  readonly reason: 'stopped' | 'max-duration';
  readonly mimeType: string;
  readonly audioBase64: string;
}

interface CaptureSnapshot {
  readonly state: 'starting' | 'recording' | 'stopping' | 'completed' | 'failed';
...
interface ActiveCapture {
  ...
  state: 'starting' | 'recording' | 'completed';
  result?: CaptureResult;
  expiryTimer?: ReturnType<typeof setTimeout>;
}

interface PageInvocationResult<T> {
  readonly ok: boolean;
  readonly value?: T;
  readonly error?: {
    readonly name?: string;
...
interface CaptureSessionLike {
  setPermissionCheckHandler(
    handler:
      | ((webContents: unknown, permission: string, requestingOrigin: string, details: PermissionDetails) => boolean)
      | null,
  ): void;
...
interface WebContentsLike {
  readonly session: CaptureSessionLike;
  executeJavaScript(script: string, userGesture?: boolean): Promise<unknown>;
  getURL(): string;
  setWindowOpenHandler(handler: () => { action: 'deny' }): void;
  on(event: 'will-navigate', listener: (event: NavigationEventLike, navigationUrl: string) => void): void;
 ...
 */
export class EditorAudioRecorderHost {
  private active: ActiveCapture | null = null;
  private capturePageCache: string | null = null;

  public constructor(private readonly port: number) {}

  public handles(pathname: string): boolean {
   ...
   */
  public handleHttpRequest(req: http.IncomingMessage, res: http.ServerResponse, pathname: string): boolean {
  ...
  private async start(request: StartRequest): Promise<Record<string, unknown>> {
    // 已完成会话只保留很短时间，用于吸收 stop/status 的并发迟到请求；
    // 新录音可以立即替换它，不会被缓存结果误判为 BUSY。
    if (this.active?.state === 'completed') {
      this.releaseActive();
    }
    if (this.active) {
    ...
    try {
      ...
      const snapshot = await invokeCapturePage<CaptureSnapshot>(captureWindow, 'start', request, true);
      this.assertCurrent(active);
      ...
      if (this.active === active) this.releaseActive();
      throw normalizeCaptureError(error);
    }
  }

  private async status(request: SessionRequest): Promise<Record<string, unknown>> {
    ...
    const snapshot = await invokeCapturePage<CaptureSnapshot>(captureWindow, 'snapshot');
    this.assertCurrent(active);
  ...
  private async stop(request: SessionRequest): Promise<Record<string, unknown>> {
    const active = this.requireSession(request.sessionId);
    if (active.state === 'completed') {
      return this.completedResponse(active);
    }
    const captureWindow = this.requireWindow(active);
    try {
      const snapshot = await invokeCapturePage<CaptureSnapshot>(captureWindow, 'stop');
      this.assertCurrent(active);
      if (snapshot.state !== 'completed') {
  ...
  private async cancel(request: SessionRequest): Promise<Record<string, unknown>> {
    const active = this.requireSession(request.sessionId);
  ...
  private async createCaptureWindow(active: ActiveCapture): Promise<BrowserWindowLike> {
    ...
    captureWindow.on('closed', () => {
      if (this.active === active) {
        active.window = null;
        if (active.state !== 'completed') this.releaseActive();
      }
    });
    ...
    const capabilities = await invokeCapturePage<{ supported: boolean; message?: string }>(
      captureWindow,
      'capabilities',
    );
    if (!capabilities.supported) {
      throw new HostError(
        'NOT_SUPPORTED',
        capabilities.message || '编辑器安全录音页不支持 getUserMedia/MediaRecorder',
        501,
      );
    }
  ...
  private requireSession(sessionId: string): ActiveCapture {
  ...
  private requireWindow(active: ActiveCapture): BrowserWindowLike {
    const captureWindow = active.window;
    if (!captureWindow || captureWindow.isDestroyed()) {
  ...
  private assertCurrent(active: ActiveCapture): void {
    if (this.active !== active) {
  ...
  private releaseActive(): void {
    ...
    if (active.window) destroyWindow(active.window);
  }

  private cacheCompleted(active: ActiveCapture, result: CaptureResult): void {
    this.assertCurrent(active);
  ...
  private completedResponse(active: ActiveCapture): Record<string, unknown> {
    return {
      ok: true,
      sessionId: active.sessionId,
      state: 'completed',
  ...
  private readCapturePage(): string {
    if (this.capturePageCache === null) {
      this.capturePageCache = readFileSync(join(__dirname, '..', 'static', 'editor-audio-capture.html'), 'utf8');
    }
...
function loadElectron(): ElectronModuleLike {
  try {
    // 仅在 Cocos Creator 扩展主进程收到 start 时加载；Vitest/普通 Node 导入本模块不会 require Electron。
    return require('electron') as ElectronModuleLike;
  } catch (error) {
    throw new HostError(
      'EDITOR_ELECTRON_UNAVAILABLE',
      `无法加载 Cocos Creator Electron 录音宿主：${error instanceof Error ? error.message : String(error)}`,
      501,
    );
  }
}

async function invokeCapturePage<T>(
  captureWindow: BrowserWindowLike,
  method: 'capabilities' | 'start' | 'snapshot' | 'stop' | 'cancel',
  argument?: unknown,
  userGesture = false,
): Promise<T> {
  if (captureWindow.isDestroyed()) {
  ...
  const result = (await captureWindow.webContents.executeJavaScript(script, userGesture)) as PageInvocationResult<T>;
  if (!result?.ok) throw pageErrorToHostError(result?.error);
  return result.value as T;
}

function validateStartRequest(value: Record<string, unknown>): StartRequest {
  ...
  if (minDurationMs < 0 || maxDurationMs <= minDurationMs || maxDurationMs > MAX_DURATION_MS) {
    throw new HostError(
      'INVALID_OPTIONS',
      `录音时长必须满足 0 <= minDurationMs < maxDurationMs <= ${MAX_DURATION_MS}`,
      400,
    );
  }
...
function validateSessionRequest(value: Record<string, unknown>): SessionRequest {
...
function validateCaptureResult(value: CaptureResult | undefined): CaptureResult {
  if (
    !value ||
...
function allowsAudioOnly(mediaTypes: readonly string[] | undefined): boolean {
...
function destroyWindow(captureWindow: BrowserWindowLike): void {
...
function pageSnapshotError(snapshot: CaptureSnapshot, code: string, fallback: string): HostError {
  return pageErrorToHostError(snapshot.error, code, fallback);
}

function pageErrorToHostError(
  error: { readonly name?: string; readonly message?: string } | undefined,
  fallbackCode = 'CAPTURE_FAILED',
  fallbackMessage = '编辑器安全录音页执行失败',
): HostError {
  ...
  switch (name) {
    ...
    case 'PermissionDeniedError':
      return new HostError(
        'PERMISSION_DENIED',
        '麦克风权限被拒绝，请在 macOS 系统设置中允许 CocosDashboard（Cocos Creator）使用麦克风',
        403,
      );
    case 'NotFoundError':
...
function normalizeCaptureError(error: unknown): HostError {
...
function normalizeHostError(error: unknown): HostError {
  return error instanceof HostError
    ? error
