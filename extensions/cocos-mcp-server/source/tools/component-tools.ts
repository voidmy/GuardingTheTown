...
export class ComponentTools implements ToolExecutor {
    ...
    private async removeComponent(nodeUuid: string, componentType: string): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            ...
            if (!component.uuid) {
                resolve({ success: false, error: `Component cid '${componentType}' on node '${nodeUuid}' has no instance UUID.` });
                return;
            }
            // 3. 官方API直接移除
            try {
            ...
            Editor.Message.request('scene', 'query-node', nodeUuid).then((nodeData: any) => {
                if (nodeData && nodeData.__comps__) {
                    const components = nodeData.__comps__.map((comp: any) => ({
                        ...
                        uuid: this.readDumpValue(comp.value?.uuid ?? comp.uuid) ?? null,
                        enabled: this.readDumpValue(comp.value?.enabled ?? comp.enabled) ?? true,
    ...
    private readDumpValue(value: any): any {
    ...
    private async setComponentProperty(args: any): Promise<ToolResponse> {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                const rawComponent = rawNodeData.__comps__[rawComponentIndex] as any;
                const uiTransformSerializedProperties: Record<string, string> = {
                    contentSize: '_contentSize',
                    anchorPoint: '_anchorPoint',
                };
                const serializedProperty = componentType === 'cc.UITransform'
                    ? uiTransformSerializedProperties[property] || property
                    : property;
                const rawPropertyDump = rawComponent.value?.[property]
                    || rawComponent.value?.[serializedProperty]
                    || rawComponent.properties?.[property]
                    || rawComponent.properties?.[serializedProperty];
                const propertyTargetUuid = nodeUuid;
                const propertyPath = rawPropertyDump?.path
                    || `__comps__.${rawComponentIndex}.${serializedProperty}`;

                // Match the Creator Inspector transaction boundary. Without a
                // recording transaction, scene:set-property can update the live
                // value without registering a Prefab override for apply-prefab.
                ...
                // Some engine properties exposed by Inspector (for example
                // UITransform.contentSize) are accessors backed by differently named
                // serialized fields. scene:set-property may resolve successfully
                // without changing them. For value types, use the extension's scene
                // component setter as a verified fallback so the engine applies the
                // public property contract itself.
                const sceneFallbackTypes = new Set([
                    'string', 'number', 'boolean', 'integer', 'float',
                    'color', 'vec2', 'vec3', 'size',
                    'colorArray', 'numberArray', 'stringArray',
                ]);
                if (!verification.verified && sceneFallbackTypes.has(propertyType)) {
                    const fallbackResult = await Editor.Message.request('scene', 'execute-scene-script', {
                        name: 'cocos-mcp-server',
                        method: 'setComponentProperty',
                        args: [nodeUuid, componentType, property, processedValue]
                    });
                    if (fallbackResult?.success !== false) {
                        await new Promise(resolve => setTimeout(resolve, 200));
                        verification = await this.verifyPropertyChange(
                            nodeUuid,
                            componentType,
                            property,
                            originalValue,
    ...
    private async attachScript(nodeUuid: string, scriptPath: string): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            ...
            try {
                const scriptAsset = await Editor.Message.request('asset-db', 'query-asset-info', scriptPath) as any;
                if (!scriptAsset?.uuid) {
                    resolve({ success: false, error: `Script asset not found: ${scriptPath}` });
                    return;
                }

                const scriptClassId = this.uuidToCompressedId(scriptAsset.uuid);
                const matchesScript = (comp: any): boolean => comp.type === scriptName || comp.type === scriptClassId;

                // create-component 使用 @ccclass 注册名；query-node 对自定义组件返回压缩后的 classId。
                const allComponentsInfo = await this.getComponents(nodeUuid);
                if (allComponentsInfo.success && allComponentsInfo.data?.components) {
                ...
                if (allComponentsInfo2.success && allComponentsInfo2.data?.components) {
                    ...
                    if (addedScript) {
                        ...
                        resolve({
                            success: false,
                            error: `Script '${scriptName}' was not found on node after addition. Expected cid: ${scriptClassId}. Available components: ${allComponentsInfo2.data.components.map((c: any) => c.type).join(', ')}`
                        });
                    }
                } else {
                    resolve({
                ...
                resolve({ success: false, error: `Failed to attach script: ${err.message}` });
            }
        });
    }
